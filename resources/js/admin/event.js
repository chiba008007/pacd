...
$(function(){
    ...
    */
    ...
    $("[name='form_type']").on("change",function(){
        if(confirm("編集内容は破棄されるので、更新後に変更を行ってください。")){
            $(this).changeMailTitle();
        }else{
...
$.fn.selectEventData = function(){
    ...
    if(_type && _event_id && _date){
        ...
        location.href = "./program?type="+_type+"&event_id="+_event_id+"&date="+_date;
    }
    return false;
};
    ...
    if(_type > 0 && _event_id > 0 && _date ){
        ...
        .then(result => {
            ...
            $.each(result[ 'list' ],function(key,item){
                ...
                if(item['enable'] == 1){
                    $("input#number-"+item['number']).prop('checked',true);
                }
                // console.log("note=>"+item['note']);
                $("select#ampm-"+item['number']).val(item['ampm']);
                $("select#start_hour-"+item['number']).val(item['start_hour']);
                $("select#start_minute-"+item['number']).val(item['start_minute']);
                $("select#end_hour-"+item['number']).val(item['end_hour']);
                $("select#end_minute-"+item['number']).val(item['end_minute']);
                $("select#presentation_id-"+item['number']).val(item['presentation_id']);
...
$.fn.selectEventDate = function(){
    ...
    if(_event_id > 0 ){
        ...
        .then(result => {
            ...
            $("select[name='date']").append($('<option>').html("-").val(""));
            $.each(result[ 'term' ],function(key,item){
                let date = item.split("-");
                let ymd = date[0]+"年"+date[1]+"月"+date[2]+"日";
                $("select[name='date']").append($('<option>').html(ymd).val(item));
            });
            ...
            if (result) {
                ...
                $("select.presentation_id").each(function(index, elem){
                    ...
                    $.each(result, function(key, row) {
                        ...
                        $(elem).append('<option value="' + row.id + '" ' + select + '>' + row.number + '</option>');
...
$.fn.getPresentation = function(key, changeNote = true) {
    ...
    if (p_id) {
        ...
        .done(function(result) {
            // console.log(result);
            if (result) {
                if(changeNote) $("#note-"+key).val(result.description);
                ...
                if (result.proceeding) {
                    ...
                    downloads.append('<a class="uk-button uk-button-default uk-width-1-2" href="' + OPEN_FILE_URL + '/' + result.number + '/proceeding/'+result.id+'?download=true">'+_txt+'</a>');
                    downloads.append('<input type="checkbox" name="disp_status1['+key+']" value="1" checked class="disp_status" />有効');
                ...
                if (result.flash) {
                    ...
                    downloads.append('<div class="uk-width-1-1">');
                    downloads.append('<a class="uk-button uk-button-default uk-width-1-2" href="' + OPEN_FILE_URL + '/' + result.number + '/flash/'+result.id+'?download=true">'+_txt+'</a>');
                    downloads.append('<input type="checkbox" name="disp_status2['+key+']" value="1" checked class="disp_status" />有効');
                    downloads.append('</div>');
                }
                ...
                if (result.poster) {
                    ...
                    downloads.append('<div class="uk-width-1-1">');
                    downloads.append('<a class="uk-button uk-button-default uk-width-1-2" href="' + OPEN_FILE_URL + '/' + result.number + '/poster/'+result.id+'?download=true">'+_txt+'</a>');
                    downloads.append('<input type="checkbox" name="disp_status3['+key+']" value="1" checked class="disp_status" />有効');
                    downloads.append('</div>');
                }
            }
        })
