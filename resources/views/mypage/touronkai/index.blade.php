...
@include('elements.pages.bannar')
            ...
            <h3 class="section__title">参加した{{ config('pacd.category.touronkai.name') }}一覧</h3>
                        ...
                        @if($attendee->event->join_enable == 1)
                            <div class="uk-width-1-6@m" >
                                @if($attendee->is_enabled_invoice == 1)
                                    ...
                                    @else
                                        <a href="{{ route('member.join.pdf', $attendee->id."/".config('pacd.category.touronkai.key')) }}" class="uk-button uk-button-default uk-background-muted" target=_blank>
                            ...
                            <tbody>
                                @foreach ($attendee->presenters as $presenter)
                                    <tr>
                                        ...
                                        <td>
                                            <a href="{{ route('touronkai_presenter.edit', $presenter->id) }}">講演者情報変更</a>
                                        </td>
                                        <td>
                                            @if (@$presenter->presentation->number)
                                                <a href="{{ route('touronkai_presenter.edit.presentation', $presenter->presentation->id) }}">講演情報・資料変更</a>
                                            @else
                                                管理者準備中
                                            @endif
                                        </td>
