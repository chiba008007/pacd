...
@section('content')
    <div id="page">
        ...
        <div class="uk-container uk-section-small uk-text-center">
            受付を完了しました。<br>
            登録されたメールアドレスに詳細情報をお送りいたします。

            <div class="uk-section-xsmall">
                <a href="{{ $edit_url }}" class="uk-button uk-button-primary">参加者情報の確認・変更</a>
            </div>

            {{--討論会以外は個人会員・会員外は受け付けない--}}

            @if($event->presenter_flag == 1)
            @if(
                (
                    $prefix == "reikai_attendee" ||
                    $prefix == "kosyukai_attendee"
                ) &&
                ($userdata->type == 4 || $userdata->type == 1))
