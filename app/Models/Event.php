...
class Event extends Model
{
    ...
    public function setRequest($obj, $request)
    {
        ...
        $obj->discountTitle = ($request->discountTitle)?$request->discountTitle:"";
        $obj->discountNote = ($request->discountNote)?$request->discountNote:"";
    ...
    public function dispParam($param)
    {
        ...
        $set['discountTitle'] = ($param->discountTitle) ?? "";
        $set['discountNote'] = ($param->discountNote) ?? "";
    ...
    public static function getEventListsCategory($category_type)
    {
        ...
        if (count($events)) {
            ...
            foreach ($events as $key => $value) {
                ...
                $lists[$value->id]['discountTitle'] = $value->discountTitle;
                $lists[$value->id]['discountNote'] = $value->discountNote;
    ...
    public static function getEventListsSchedule()
    {
        ...
        $now = date("Y-m-d");
        $events = Event::select([
            'events.id', 'events.sponser', 'events.name', 'events.discountFlag', 'events.discountText', 'events.discountRate', 'events.discountTitle', 'events.discountNote','events.event_info', 'events.category_type', 'events.code', 'events.event_type', 'events.coworker', 'events.date_start', 'events.date_end', 'events.place', 'events.event_address', 'events.party', 'events.party_address', 'events.bank_name', 'events.bank_code', 'events.sanka_explain', 'events.other'
        ])
            ->where('events.status', 1)
            // ->where('events.date_start','>=', $now)
