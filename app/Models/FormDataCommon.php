...
class FormDataCommon extends Model
{
    use HasFactory;

    protected $table = 'form_data_common';
     ...
     * @return void
     */
    static public function createFromInputData(array $data, User $user)
    {
        try {
     ...
     * @return void
     */
    static public function updateFormInputData(array $data, User $user)
    {
        $custom_data = $user->custom_form_data->keyBy('form_input_value_id');
        try {
            foreach ($data as $input_id => $input) {
                if (isset($input['data'])) {
                    foreach ($input['data'] as $value_id => $val) {
