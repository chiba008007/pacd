...
class UpdateController extends Controller
{
    ...
    public function __construct()
    {
        ...
        if ($current) {
            ...
            // 例会の参加者でない場合エラー
            if ($this->attendee->event->category_type != config('pacd.category.reikai.key')) {
                abort(404);
            }
        }
    }
    ...
    public function edit($attendee_id)
    {
        ...
        $set['inputs'] = FormInput::where(['form_type' => $this->form['key'], 'is_display_published' => true])->whereIN('event_id',[0,$this->attendee->event_id])->get();
