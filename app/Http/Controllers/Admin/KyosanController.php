...
class KyosanController extends Controller
{
    ...
    {
        //カテゴリタイプの指定
        $this->category_prefix = config('pacd.category.kyosan.prefix');
        $this->category_type = config('pacd.category.kyosan.key');
        $this->category_title = config('pacd.category.kyosan.name');
    ...
    public function event_add(Request $request){
        ...
        if($this->Common->setEvent($request)){
            return redirect()->back()->with('status', '企業協賛情報の更新を行いました');
        }else{
            return redirect()->back()->with('status', '企業協賛情報の登録に失敗しました。');
        }
    }
     ...
     * プログラム設定
     */
    public function event_program($id=0,request $request){
